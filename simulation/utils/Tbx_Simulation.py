...
def generate_weekly_consumption_patterns(base_demand, n_intervals = 48, n_days = 7,
    ...
    """
    Generates weekly consumption patterns (7 days) using the daily consumption function, based on monthly consumption and base demand.
    ...
    for _ in range(n_days):
        daily_pattern = generate_daily_consumption_pattern(base_multiplier = base_demand,
                                                           daily_consumption = base_demand,
...
def generate_daily_consumption_pattern(base_multiplier = 1.0, daily_consumption = 1.0, n_intervals = 48,
    ...
    # Example pre-checks for NaN, Inf, and zero total
    if np.isnan(daily_pattern_total).any():
        print("Warning: NaN detected in daily_pattern_total at line 326.")
    elif np.isinf(daily_pattern_total).any():
        print("Warning: Inf detected in daily_pattern_total at line 326.")
    elif np.any(daily_pattern_total == 0):
        print("Warning: Zero detected in daily_pattern_total at line 326.")
...
def rename_elements(epanet_object):
    id_mapping = {
        wntr.network.elements.Junction: 'J-',
        wntr.network.elements.Reservoir: 'R-',
        wntr.network.elements.Pipe: 'P-'
    }
    
    element_id, element = epanet_object
    prefix = None
    
    for element_type, pref in id_mapping.items():
        if isinstance(element, element_type):
            prefix = pref
            break
    
    if prefix and prefix not in element_id:
        new_id = prefix + element_id
        # print(f"Original ID: {element_id}, Object Type: {type(element).__name__}, New ID: {new_id}")
    else:
        print(f"Object type {type(element).__name__} not found in mapping.")
        raise ValueError()

    if isinstance(element, wntr.network.elements.Pipe):
        if prefix not in element._link_name:
            element._link_name = prefix + element._link_name
    else:
        if prefix not in element._name:
            element._name = prefix + element._name
        
    return new_id, element
...
def generate_consumption_patterns(data_consumption,
    ...
    for node, base_consumption in zip(data_consumption['node_id'], data_consumption['consumption']):
        ...
        for consumption in consumption_record:
            ...
            base_peaks = np.array([morning_peak, afternoon_peak, evening_peak, night_consumption])
        ...
        transition = 5
        wght_prev = [i for i in range (transition , 0, -1)]
        wght_curr = [i for i in range (1, transition + 1)]
        ...
        consumption_patterns[id_time][node]['base_demand'] = base_demand
