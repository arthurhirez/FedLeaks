...
from utils.Toolbox_visualization import load_and_scale_data, combine_latents, plot_latent_heatmap, \
    plot_time_series_and_latents
...
def main():
    ...
    for scenario in scenarios:
        ...
        for epoch in iterator:
            ...
            aux_df.columns = ['epoch', 'label', 'client_2', 'client_1', 'cosine', 'manhattan', 'wavelet', 'dft',
                              'autocorr']

            df_exp_proto = pd.concat([df_exp_proto, aux_df[df_exp_proto.columns.tolist()]])
            df_exp_proto = df_exp_proto.sort_values(by=['epoch', 'label', 'client_1', 'client_2']).reset_index(
                drop=True)
            # df_exp_proto['id'] = tag
    ...
    for scenario in scenarios:
        pd.concat(all_latents[scenario], ignore_index=True).to_parquet(f"{results_dir}/{scenario}_latent_space.parquet",
                                                                       index=False)
        pd.concat(all_protos[scenario], ignore_index=True).to_parquet(f"{results_dir}/{scenario}_proto.parquet",
                                                                      index=False)
        if args.save_extras: pd.concat(all_dists[scenario], ignore_index=True).to_parquet(
            f"{results_dir}/{scenario}_distribution.parquet", index=False)

        if args.generate_viz:
            pd.concat(all_pca[scenario], ignore_index=True).to_parquet(f"{results_dir}/{scenario}_pca.parquet",
                                                                       index=False)
            pd.concat(all_umap[scenario], ignore_index=True).to_parquet(f"{results_dir}/{scenario}_umap.parquet",
                                                                        index=False)
