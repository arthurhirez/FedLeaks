...
class FederatedModel(nn.Module):
    ...
    def aggregate_nets(self, freq=None):
        ...
        if self.args.averaging == 'weight':
            online_clients_data = [self.trainloaders[online_clients_index] for online_clients_index in online_clients]
            online_clients_len = [data.shape[0] for data in online_clients_data]
        ...
        self.weight_history.append(copy.deepcopy(self.global_net.state_dict()))
    ...
    #         # online_clients_dl = [self.trainloaders[online_clients_index][epoch_index] for online_clients_index in online_clients]
    ...
    #             first = False
    #             for key in net_para:
    #                 global_w[key] = [w * freq[index] for w in net_para[key]]
    #         else:
    #             for key in net_para:
    #                 global_w[key] = [gw + w * freq[index] for gw, w in zip(global_w[key], net_para[key])]
    #
    #     global_net.load_state_dict(global_w)
    #
    #     for _, net in enumerate(nets_list):
